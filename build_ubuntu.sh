...
# Function to map Ubuntu architecture to zellij release name
get_zellij_release() {
    local arch=$1
    case "$arch" in
        "amd64")
...
build_architecture() {
    ...
    # Build packages for all supported Ubuntu distributions
    declare -a arr=("jammy" "noble" "questing" "resolute")

    for dist in "${arr[@]}"; do
        FULL_VERSION="$zellij_VERSION-${BUILD_VERSION}~${dist}_${build_arch}_ubu"
        echo "  Building $FULL_VERSION"

        if ! docker build . -f Dockerfile.ubu -t "zellij-ubuntu-$dist-$build_arch" \
        ...
        id="$(docker create "zellij-ubuntu-$dist-$build_arch")"
